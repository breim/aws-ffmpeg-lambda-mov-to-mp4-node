      ...
      await Promise.all(
        records.map(async record => {
          ...
          try {
              ...
              const { stdout, stderr } = await exec(`/opt/ffmpeg/ffmpeg -i /tmp/${video_name} -vcodec libx264 -pix_fmt yuv420p -profile:v baseline -level 3 -f mp4 /tmp/${video_base_name}.mp4`);

              const outputFileBuffer = await fs.readFileSync(`/tmp/${video_base_name}.mp4`);

              await s3.putObject({
                Body: outputFileBuffer,
                Bucket: bucket_name,
                ContentType: 'binary/octet-stream',
                Key: `conversions/${video_base_name}.mp4`
              }).promise();
